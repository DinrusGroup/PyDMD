        ...
        if os.path.exists(filename):
            ...
            for line in file:
                ...
                if codeExists:
                    ...
                    line = line.replace("ogre.StringConverter.toString", "str")
                    line = line.replace("ogre.StringConverter.parseInt", "str")
                    line = line.replace("ogre.StringConverter.parseBool", "str")
                    ...
                    if (line.lower().find("ogre") != -1) and (ogreImport not in imports):
                        imports.append(ogreImport)
                    if (line.lower().find("ois") != -1) and (oisImport not in imports):
                        imports.append(oisImport)
                    if (line.lower().find("ogrenewt") != -1) and (newtImport not in imports):
                        imports.append(newtImport)
                    if (line.lower().find("cegui") != -1) and (ceguiImport not in imports):
                        imports.append(ceguiImport)
                
                    if self.debug: print "2: " + line.strip("\n") + "\n-----------"
            ...
            raise IOError(0, filename, "The script '"+filename+"' could not be found.")
    

if __name__ == "__main__":
    parser = CPP2PY()
    parser.convert("source.cpp", "source.py")
